...
export default function App() {
  ...
  const isAuthCallback = useMemo(() => {
    ...
    // Only trigger on the explicit callback path OR when Supabase puts an
    // access_token in the hash WITH a type parameter (magic link redirect).
    // A bare hash or fragment on another page must not trigger this.
    const h = window.location.hash;
    const onCallbackPath = window.location.pathname === "/auth/callback";
    const hasSupabaseToken = h.includes("access_token=") && (h.includes("type=magiclink") || h.includes("type=recovery") || h.includes("type=signup"));
    return onCallbackPath || hasSupabaseToken;
  }, []);
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      ...
      // Only auto-redirect to app if they have an active session AND
      // they didn't specifically navigate to the root (which should show landing).
      // We respect their choice to be on the landing page.
      const cameDirectly = window.location.pathname === "/" && !window.location.search;
      if (!publicSlug && s.meta?.user && !cameDirectly) setScreen("app");
    };
  ...
  // Show landing immediately — don't wait for store.load() to complete.
  // This means the landing is always instant. We transition away only after
  // load() confirms an existing session.
  if (publicSlug) {
    ...
    return <PublicRoute slug={publicSlug} state={state} store={store} />;
  }
